...
2 metal spheres displaying "fuzzy reflections" using random points in a unit sphere similar to how lambertian reflections reflect light on random points on the surface of a unit sphere. The left has a much lower "fuzz" factor (0 to be exact) than the right sphere. The middle sphere is a matte sphere. At this point in the ray tracer we are able to give colors to the spheres as well as materials with certain properties to the spheres themselves.
